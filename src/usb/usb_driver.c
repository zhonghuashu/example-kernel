 ...
 *        Test with Raspberry Pi 3B+ / Mobile phone USB connection.
...
*/
static int example_usb_probe(struct usb_interface *interface,
...
*/
static void example_usb_disconnect(struct usb_interface *interface)
{
...
const struct usb_device_id example_usb_table[] = {
...
MODULE_DEVICE_TABLE(usb, example_usb_table);
...
module_usb_driver(example_usb_driver);

#else
static int __init example_usb_init(void)
{
    // register the USB device
    return usb_register(&example_usb_driver);
}

static void __exit example_usb_exit(void)
{
    // deregister the USB device
    usb_deregister(&example_usb_driver);
}

module_init(example_usb_init);
module_exit(example_usb_exit);
#endif
